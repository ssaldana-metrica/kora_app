...
const styles = StyleSheet.create({
  ...
  logoText: { fontSize: 22, fontFamily: 'Helvetica-Bold', color: '#0d3d7a', letterSpacing: 2 },
  logoSubtitle: { fontSize: 8, color: '#64748b', marginTop: 2, letterSpacing: 0.5 },
  headerRight: { flexDirection: 'column', alignItems: 'flex-end' },
  headerDate: { fontSize: 9, color: '#64748b' },
  ...
  label: { fontSize: 9, color: '#64748b', width: 140, fontFamily: 'Helvetica-Bold' },
  ...
  resumenText: { fontSize: 9, color: '#334155', lineHeight: 1.6 },
  table: { marginTop: 6 },
  tableHeader: {
    flexDirection: 'row',
  ...
  tableCell: { fontSize: 8, color: '#334155', flex: 1 },
  tableCellGreen: { fontSize: 8, color: '#16a34a', flex: 1, fontFamily: 'Helvetica-Bold' },
  tableCellAmber: { fontSize: 8, color: '#d97706', flex: 1, fontFamily: 'Helvetica-Bold' },
  tableCellRed:   { fontSize: 8, color: '#dc2626', flex: 1, fontFamily: 'Helvetica-Bold' },
  sintomaChip: {
  ...
  sintomaText: { fontSize: 8, color: '#92400e' },
  sintomasRow: { flexDirection: 'row', flexWrap: 'wrap', marginTop: 4 },
  footer: {
    position: 'absolute',
    bottom: 24,
    left: 45,
...
interface Registro {
  ...
  tomo_medicamento: boolean          // ✅ nombre correcto
...
function calcularAdherenciaSemanal(registros: Registro[]) {
  ...
  for (let i = 0; i < ordenados.length; i += 7) {
    ...
    const tomados = semana.filter(r => r.tomo_medicamento).length  // ✅
    semanas.push({
...
function KoraPDF({ pacienteNombre, pacienteEnfermedad, medicoNombre, codigoMedico, registros, resumenIA }: Props) {
  ...
  const notasPaciente = registros.filter(r => r.notas?.trim()).slice(0, 5)
  const periodoInicio = registros.length > 0 ? new Date(registros[registros.length - 1].fecha).toLocaleDateString('es-PE') : '—'
  const periodoFin    = registros.length > 0 ? new Date(registros[0].fecha).toLocaleDateString('es-PE') : '—'
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.row}><Text style={styles.label}>Nombre:</Text><Text style={styles.value}>{pacienteNombre}</Text></View>
          {pacienteEnfermedad && <View style={styles.row}><Text style={styles.label}>Diagnóstico:</Text><Text style={styles.value}>{pacienteEnfermedad}</Text></View>}
          <View style={styles.row}><Text style={styles.label}>Período del reporte:</Text><Text style={styles.value}>{periodoInicio} — {periodoFin}</Text></View>
          <View style={styles.row}><Text style={styles.label}>Total de registros:</Text><Text style={styles.value}>{total} días</Text></View>
        </View>
        ...
        {resumenIA ? (
          <View style={styles.section}>
            ...
            <View style={styles.resumenBox}><Text style={styles.resumenText}>{resumenIA}</Text></View>
          </View>
        ) : null}
        ...
        <View style={styles.section}>
          ...
          <View style={styles.row}>
            ...
            <Text style={[styles.value, adherenciaTotal >= 80 ? { color: '#16a34a', fontFamily: 'Helvetica-Bold' } : adherenciaTotal >= 50 ? { color: '#d97706', fontFamily: 'Helvetica-Bold' } : { color: '#dc2626', fontFamily: 'Helvetica-Bold' }]}>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.table}>
            <View style={styles.tableHeader}>
              {['Semana', 'Días registrados', 'Días tomó medic.', 'Adherencia'].map(h => (
            ...
            {semanas.map((s, i) => (
              <View key={i} style={[styles.tableRow, i % 2 !== 0 ? styles.tableRowAlt : {}]}>
                ...
                <Text style={s.pct >= 80 ? styles.tableCellGreen : s.pct >= 50 ? styles.tableCellAmber : styles.tableCellRed}>
                  {s.pct}%
                </Text>
              </View>
            ))}
            ...
            <View style={styles.sintomasRow}>
              {sintomas.map((s, i) => (
                <View key={i} style={styles.sintomaChip}>
                  <Text style={styles.sintomaText}>{s.nombre}: {s.count} días ({s.pct}%)</Text>
                </View>
              ))}
            </View>
          </View>
          ...
          <View style={styles.section}>
            ...
            {notasPaciente.map((r, i) => (
              <View key={i} style={[styles.row, { marginBottom: 6 }]}>
                <Text style={styles.label}>{new Date(r.fecha).toLocaleDateString('es-PE')}:</Text>
        ...
        <View style={styles.footer} fixed>
          <Text style={styles.footerText}>Generado por KORA — plataforma de seguimiento clínico entre consultas</Text>
...
export default function ExportarPDF(props: Props) {
  const nombreArchivo = `KORA_${props.pacienteNombre.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.pdf`
