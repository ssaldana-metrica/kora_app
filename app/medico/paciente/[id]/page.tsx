...
interface Registro {
  ...
  mareos: boolean
  hinchazon: boolean
  tomo_medicamento: boolean        // ✅ columna real en Supabase
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-100 bg-gray-50">
          <h3 className="font-semibold text-gray-900 flex items-center gap-2">
            <Brain size={16} className="text-[#0d3d7a]" /> Agente KORA
          </h3>
          <p className="text-xs text-gray-500 mt-0.5">Pregúntale cualquier cosa sobre {paciente.nombre}</p>
        </div>
            ...
            {mensajes.map((m, i) => (
              <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[85%] px-4 py-2.5 rounded-xl text-sm leading-relaxed ${
                  m.role === 'user' ? 'bg-[#0d3d7a] text-white' : 'bg-gray-100 text-gray-800'
                }`}>{m.content}</div>
              </div>
            ))}
            {cargandoAgente && (
        ...
        {mensajes.length === 0 && (
          <div className="px-4 py-3 flex flex-wrap gap-2">
            {sugeridas.map(p => (
              <button key={p} onClick={() => enviar(p)} className="text-xs bg-blue-50 text-[#0d3d7a] border border-blue-200 px-3 py-1.5 rounded-full hover:bg-blue-100 transition-colors">
                {p}
              </button>
            ))}
          </div>
        ...
        <div className="px-4 pb-4 pt-2 border-t border-gray-100 flex gap-2">
          ...
          <button
            ...
            className="bg-[#0d3d7a] text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-[#0b3268] disabled:opacity-40 transition-colors"
          >
            Enviar
          </button>
        </div>
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
        <h3 className="font-semibold text-gray-900 mb-1 flex items-center gap-2"><Activity size={16} className="text-[#0d3d7a]" />Presión arterial (últimos 30 días)</h3>
        ...
        {datosPresion.length > 0 ? (
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={datosPresion}>
              ...
              <Line type="monotone" dataKey="sistolica" stroke="#0d3d7a" strokeWidth={2} dot={{ r: 3 }} name="Sistólica" />
              <Line type="monotone" dataKey="diastolica" stroke="#60a5fa" strokeWidth={2} dot={{ r: 3 }} name="Diastólica" />
              <Legend />
            </LineChart>
          </ResponsiveContainer>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
        <h3 className="font-semibold text-gray-900 mb-1 flex items-center gap-2"><Pill size={16} className="text-[#0d3d7a]" />Adherencia al medicamento por semana</h3>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
        <h3 className="font-semibold text-gray-900 mb-1 flex items-center gap-2"><TrendingUp size={16} className="text-[#0d3d7a]" />Bienestar general (escala 1–5)</h3>
        ...
        <ResponsiveContainer width="100%" height={180}>
          <LineChart data={datosBienestar}>
            ...
            <Tooltip />
            <Line type="monotone" dataKey="bienestar" stroke="#10b981" strokeWidth={2} dot={{ r: 3 }} name="Bienestar" />
          </LineChart>
        </ResponsiveContainer>
      </div>
    </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-200">
              {['Fecha', 'Presión', 'Medicamento', 'Síntomas', 'Bienestar', 'Notas'].map(h => (
...
function TabDocumentos({ documentos }: { documentos: Documento[] }) {
  ...
  return (
    <div className="space-y-3">
      {documentos.map(doc => (
        <div key={doc.id} className="bg-white rounded-xl border border-gray-200 p-4 flex items-center justify-between gap-4 hover:shadow-sm transition-shadow">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-0.5">
                ...
                {doc.fecha_documento && <span className="text-xs text-gray-400">{new Date(doc.fecha_documento).toLocaleDateString('es-PE')}</span>}
                {doc.procesado_ia && <span className="text-xs bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-full font-medium">✓ Procesado por IA</span>}
              </div>
            </div>
          </div>
          <a href={doc.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-xs text-[#0d3d7a] hover:underline flex-shrink-0">
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 shadow-sm">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-4">
          <div className="grid grid-cols-3 gap-4 sm:gap-6">
            ...
            <div className="text-center">
              <p className={`text-2xl font-bold ${adherencia >= 80 ? 'text-emerald-600' : adherencia >= 50 ? 'text-amber-500' : 'text-red-500'}`}>{adherencia}%</p>
            ...
            {tabs.map(tab => (
              <button key={tab.key} onClick={() => setTabActivo(tab.key)}
                className={`flex items-center gap-2 px-4 py-4 text-sm font-medium border-b-2 transition-colors ${
                  tabActivo === tab.key ? 'border-[#0d3d7a] text-[#0d3d7a]' : 'border-transparent text-gray-500 hover:text-gray-700'
                }`}>
                {tab.icon}
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-6">
        {tabActivo === 'resumen' && <TabResumen paciente={paciente} registros={registros} documentos={documentos} medicoNombre={medicoNombre} codigoMedico={codigoMedico} />}
